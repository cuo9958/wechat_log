 ...
 * @param {*} len 字节长度
 */
function addToIndex(title, start, len, id) {
 ...
 * @param {*} len 字节数
 */
function addToWait(title, str, len, time) {
    BackList.push({
        title,
        str,
        len,
...
function writeWait() {
    ...
    fs.getFileInfo({
        ...
        complete: function (res) {
            addToIndex(data.title, res.size, data.len, data.time)
...
export function addLog(title, data, opts, time = Date.now()) {
    ...
    fs.getFileInfo({
        filePath: path,
        complete: function (res) {
            addToIndex(title, res.size, len, time)
 ...
 * @param {*} len 每页条数
 */
export function getLogList(pageIndex = 0, len = 30) {
    const start = pageIndex * len
    const end = start + len
    const data = IndexList.slice(start, end)
    return data
}
/**
 * 获取对应日志的详情
...
export function getLogInfo(id) {
    // getFileInfo()
    const data = IndexList.find((item) => item.id + '' === id)
    if (!data) return null
